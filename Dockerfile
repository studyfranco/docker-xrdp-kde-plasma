...
RUN set -x \
    ...
    && DEBIAN_FRONTEND=noninteractive apt-get install -y --allow-unauthenticated deb-multimedia-keyring tar wget curl pigz

RUN set -x \
    && DEBIAN_FRONTEND=noninteractive apt-get install -y jq mpv kde-plasma-desktop krename gPRename xrdp firefox mediainfo mkvtoolkit mkvtoolnix-gui ffmpeg ldap-utils --no-install-recommends\
    && apt clean \
