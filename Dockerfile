...
RUN set -x \
    ...
    && DEBIAN_FRONTEND=noninteractive apt-get install -y --allow-unauthenticated -t testing deb-multimedia-keyring \
    && apt update

RUN set -x \
    && DEBIAN_FRONTEND=noninteractive apt-get install -y tar wget curl pigz jq mpv kde-plasma-desktop krename gPRename xrdp firefox mediainfo mkvtoolkit mkvtoolnix-gui ffmpeg ldap-utils --no-install-recommends\
    && apt clean \
